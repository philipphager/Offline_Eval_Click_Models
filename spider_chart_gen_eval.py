...
configurations = {"TCM" : ["PBM", "UBM", "DBN", "NCM", "ARM", "NEHBM"], 
                    ...
                    "TRP" : {"PL-oracle" : "oracle", "PL-λMART" : "-lambdamart", "PL-bm25" : "-bm25"},
                    "TEP" : {"oracle" : "logged-relevances", "λMART" : "logged-lambdamart", "bm25" : "logged-bm25" , "random" : "random"}}
...
def make_spider(values, intervals, configs):
    ...
    for i in range(n_ax):
        ...
        ax = plt.subplot(2, math.ceil(n_ax/2),1 + i, polar=True)
        ax.grid(linewidth = 1)        
        ...
        filtered_teps = ["λMART", "bm25", "random", "oracle", "bm25", "random","oracle", "λMART", "random"] * 3
        ...
        for m, (trp, angle) in enumerate(zip(TRPs * n_icm, np.rad2deg(angles[0:-1:n_tep]))) :
            ...
            new_angle = angle + np.rad2deg(4 * np.pi / float(n_angles))
